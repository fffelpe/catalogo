...
async function descobrirAbaNoticias() {
  ...
  const titulos = (resposta.data.sheets || [])
    .map((aba) => aba.properties?.title)
    .filter(Boolean);

  const encontrada = titulos.find((titulo) =>
    titulo.toLocaleUpperCase("pt-BR").startsWith("NOTÍCIAS E OUTRAS NOTÍCIAS")
  );

  if (!encontrada) {
    throw new Error("Não foi encontrada a aba de notícias na planilha configurada.");
  }
...
async function carregarNoticias() {
  ...
  for (const linha of linhas) {
    // Estrutura da origem: PGM, ID, DATA, ASSUNTO, UF, REPÓRTER,
    // AFILIADA / EMISSORA, EDITORIA, PROGRAMA.
    const id = normalizarId(linha[1]);
    if (!id) continue;

    const registro = [
      id,
      limparTexto(linha[3]), // ASSUNTO -> DESCRIÇÃO
      limparTexto(linha[2]), // DATA
      limparTexto(linha[4]), // UF -> LOCAL
      limparTexto(linha[5]), // REPÓRTER
      limparTexto(linha[6]), // AFILIADA / EMISSORA
      limparTexto(linha[8]) || "AGROCULTURA", // PROGRAMA
      limparTexto(linha[7]), // EDITORIA
    ];
    ...
    // Se o mesmo ID aparecer mais de uma vez na origem, consolida sem duplicar.
    const atual = porId.get(id);
    porId.set(
      id,
      atual.map((valor, indice) => registro[indice] || valor)
    );
  }
...
async function main() {
  console.log("Iniciando integração de NOTÍCIAS E OUTRAS NOTÍCIAS QUE FORAM AO AR...");
  ...
  for (const noticia of noticias) {
    ...
    if (existente) {
      ...
      if (JSON.stringify(dadosNovos) !== JSON.stringify(existente.dados)) {
        atualizacoes.push({
          range: `imgs!A${existente.linhaPlanilha}:H${existente.linhaPlanilha}`,
          values: [dadosNovos],
        });
  ...
  console.log("Integração de notícias finalizada.");
  console.log(`Atualizados: ${atualizados}`);
  console.log(`Novos: ${novos}`);
  console.log(`Total de IDs da origem processados: ${noticias.length}`);
}
