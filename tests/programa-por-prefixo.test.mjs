...
import { programaPorMediaId, programaPorListaDeIds, normalizarMediaId } from "../scripts/programa-por-prefixo.mjs";
...
test("preserva programa existente quando a família do Media ID é desconhecida", () => {
  assert.equal(
    programaPorListaDeIds(["9999B001234"], "RODA VIVA"),
    "RODA VIVA"
  );
});

test("não preserva placeholder PROGRAMA NAO DEFINIDO quando a família passa a ser conhecida", () => {
  assert.equal(
    programaPorListaDeIds(["1452P000279"], "PROGRAMA NAO DEFINIDO"),
    "AGROCULTURA"
  );
});
